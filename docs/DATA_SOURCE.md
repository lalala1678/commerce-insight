...
- 原始数据不提交 Git；仓库提供下载指引与版本摘要。
- 基础版 sample_data 为自行创建的合成数据，与 Olist 无关。

当前工作环境已下载真实 ZIP，解压文件放在 data/raw/，该目录被 .gitignore 忽略。第 1 周对全部 9 个文件完成只读审计；主线 ETL 使用其中 8 个文件建立真实分析表，与 `demo_` 合成样例表隔离。其他人克隆仓库后需自行从发布页下载并解压；仓库不包含本地数据库。

该样本来自巴西多卖家平台，包含订单、商品与客户所在地区等历史记录，不是一家店的完整经营数据，也不代表中国市场。它能检验分析口径与页面流程，不能证明中国商家愿意使用、持续使用或付费。
...
- 当前用途限定为非商业学习与作品展示，不将此核对理解为对未来商业用途的授权。若将来试点收费服务或接入商家经营数据，应先取得适用的数据使用授权，另行评估隐私、权限和部署要求；不能直接把这份 Olist 数据当作商业产品的数据源。数据许可不能直接代替项目代码许可；当前未替代码单独选择许可证。
