...
在线页沿用经营概览、商品与履约、客户分析、经营报告四个页面。它读取当前仓库已提交的 `frontend/public/static-data/` 汇总快照，仅支持页面下拉菜单中的七个观察范围。完整本地后台仍通过 FastAPI 和数据库查询任意可用日期、品类、州，并提供订单详情。
...
| 2018 年 2–7 月，全部 | 同一套指标的长观察窗；RFM 同时包含末次购买距截止日 ≤30 天和 >30 天的窗口购买客户 |
| 2018 年 7 月，SP | 地区筛选例子 |
| 2018 年 7 月，床上与卫浴＋SP | 同时筛选品类和地区 |
| 2018 年 7 月 23–29 日 | 完整周报例子 |
| 2017 年 11 月 20–26 日 | 历史异常提示核查例子 |

每个范围保存三种趋势粒度及客户、经营报告汇总，共 36 个 JSON 文件，另有哈希清单。七个范围都能在四个页面使用，不单独切换客户分析口径。导出器使用与本地接口同一套分析函数，核对三个模块的商品金额、订单数与客户数，强制校验已知的 7 月、品类＋SP 和长窗口 RFM 数据。清单中的 SHA-256 能发现截断或意外修改；文件都是整型分金额和聚合维度。导出器递归拒绝订单 ID、稳定客户 ID、原始评价文本、地址与地理坐标等字段。静态站不提供订单明细。RFM 只覆盖当前观察窗有购买的客户，不能代表全部历史未购买客户。
